    ...
    async (req, reply) => {
      ...
      try {
        ...
        if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === 'P2002') {
          return reply.code(409).send({ error: 'A user with that email already exists in this organization' });
        }
        throw err;
      }
    },
  ...
  router.patch(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === 'P2002') {
          return reply.code(409).send({ error: 'Email already in use in this organization' });
        }
        throw err;
      }
    },
