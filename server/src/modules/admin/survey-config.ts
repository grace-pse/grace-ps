// Admin survey-config routes (P2). Gated on `surveys:admin` — tenants
...
export default async function adminSurveyConfigRoutes(app: FastifyInstance) {
  ...
  router.get(
    '/config',
    {
      ...
      schema: {
        tags: ['admin-surveys'],
        summary: 'Get the tenant survey config (enabled types + custom types)',
        security: [{ bearerAuth: [] }],
    ...
    async (req) => {
      ...
      let row = await prisma.tenantSurveyConfig.findUnique({ where: { tenantId } });
      if (!row) {
        row = await prisma.tenantSurveyConfig.create({
          data: {
            tenantId,
  ...
  router.put(
    '/config',
    {
      ...
      schema: {
        tags: ['admin-surveys'],
        summary: 'Update tenant survey config',
        security: [{ bearerAuth: [] }],
    ...
    async (req, reply) => {
      ...
      const data: Prisma.TenantSurveyConfigUpdateInput = {};
      ...
      const row = await prisma.tenantSurveyConfig.upsert({
        where: { tenantId },
        update: data,
        create: {
          tenantId,
  ...
  router.get(
    '/defaults',
    {
      ...
      schema: {
        tags: ['admin-surveys'],
        summary: 'List per-asset-type survey defaults for this tenant',
        security: [{ bearerAuth: [] }],
  ...
  // ── PUT /defaults ─────────────────────────────────────────
  // Upsert by (tenant, assetType, surveyType). If isDefault=true, clear
  // any other rows of the same (tenant, assetType) so there is at most
  // one default per asset type.
  router.put(
    '/defaults',
    {
      onRequest: [app.authenticate, requirePermission('surveys:admin')],
    ...
    async (req, reply) => {
      ...
      if (req.body.templateId) {
        ...
        if (!t) return reply.code(404).send({ error: 'Template not found or not accessible' });
      }

      const row = await prisma.$transaction(async (tx) => {
        if (req.body.isDefault) {
          await tx.assetTypeSurveyDefault.updateMany({
            where: { tenantId, assetType: req.body.assetType, isDefault: true },
            data: { isDefault: false },
          });
        }
        return tx.assetTypeSurveyDefault.upsert({
          where: {
            tenantId_assetType_surveyType: {
              tenantId,
  ...
  router.get(
    ...
    async (req, reply) => {
      ...
      const cluster = await prisma.assetCluster.findFirst({
        where: { id: req.query.clusterId, tenantId },
        include: {
