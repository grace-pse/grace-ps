...
type RecommendationRow = {
  id: string; tenantId: string; assessmentId: string;
...
export default async function recommendationRoutes(app: FastifyInstance) {
  ...
  router.post(
    ...
    async (req, reply) => {
      ...
      const a = await prisma.assessment.findFirst({
        where: { id: req.params.id, tenantId }, select: { id: true, tenantId: true },
      });
      if (!a) return reply.code(404).send({ error: 'Assessment not found' });
  ...
  router.patch(
    ...
    async (req, reply) => {
      ...
      const existing = await prisma.recommendation.findFirst({
        where: { id: req.params.recId, assessment: { tenantId } },
        select: { id: true },
      });
