...
// Adds a second assessment (IN_REVIEW state) to the existing block-b-smoke tenant
// so Block C (review workflow) can be smoke-tested. Idempotent: wipes any prior
// IN_REVIEW row for this tenant+title before creating a new one.
...
async function main() {
  ...
  const lead = await prisma.user.findFirst({ where: { tenantId: org.id, role: 'LEAD_ASSESSOR' } });
  const admin = await prisma.user.findFirst({ where: { tenantId: org.id, role: 'ADMIN' } });
  if (!lead || !admin) throw new Error('lead or admin user missing in tenant');

  // Wipe prior copy (by title) — lets us rerun the seed cleanly.
  const prior = await prisma.assessment.findFirst({ where: { tenantId: org.id, title: TITLE } });
  if (prior) {
  ...
  let osloSite = await prisma.asset.findFirst({ where: { tenantId: org.id, name: 'Oslo Office' } });
  if (!osloSite) {
    osloSite = await prisma.asset.create({
      data: {
        tenantId: org.id, name: 'Oslo Office', assetType: 'SITE',
  ...
  let osloFloor = await prisma.asset.findFirst({ where: { tenantId: org.id, name: 'Demo floor', parentId: osloSite.id } });
  if (!osloFloor) {
    osloFloor = await prisma.asset.create({
      data: {
        tenantId: org.id, parentId: osloSite.id, name: 'Demo floor', assetType: 'FLOOR',
